...
level               equ   Explode_f + 1                            ; only does levels 1-99 then shows Infinity sign after that 
enemycnt            equ   level + 1
temp                equ   enemycnt + 2                            ; generic 2 byte temps 
temp1               equ   temp + 2 
temp2               equ   temp1 + 2 
temp3               equ   temp2 + 2 
temp4               equ   temp3 + 2 
spawntemp           equ   temp4 + 1 
                        
alley1e             equ   spawntemp + 1       ; is there a monster in the alley? (Exists?) 
...
; tempo limite (timeout) antes do reaparecimento por caminho
alley0to            equ   alley6e + 1 
alley1to            equ   alley0to + 1 
alley2to            equ   alley1to + 1 
alley3to            equ   alley2to + 1 
alley4to            equ   alley3to + 1 
alley5to            equ   alley4to + 1 
alley6to            equ   alley5to + 1 
...
; don't even do spawn logic if maximum enemies are out or ship is dead	
				   lda 		level
				   cmpa 	#7						;é o nivel 7 
				   blt 		dontfix                 ; se n for o level 7 avança para 'dontfix'
				   lda      #7
				   bra		doenemycntchk
				   
dontfix			
				   ldx 		#max_enemys_t           ; tabela n maximo de inimigos para cada nivel
				   lda 		a,x
				   
				   				
doenemycntchk				   
				   cmpa      enemycnt    			; é igual ao que existe atualmente ( numero inimigos atual)      
				   lble      no_new_enemy			; se sim entao n cria novos inimigos
				   ;gera novo inimigo
				   lda       #15					;numero de tentativas de criar um novo inimigo
				   sta		 temp
randloop
				   dec      temp					;loop com temp=15
				   lbeq     no_new_enemy			;se fizer mais de 14 loops, aborta vai decrementando 15 até 0 
				   
				   jsr      Random					;escolhe um caminho (alley), loop ate ser encontrado
				   anda     #%00000111              ; de 0 a 7 se estiver ocupado ou 7 tente novamente
				   cmpa     #7
				   beq      randloop
				   sta      spawntemp
				   ldx      #alleye_t				;tabela para verificar se existre monstro no caminho
				   lsla
				   ldx       a,x
				   ldb       ,x
				   bne       randloop
				   
				   ldx       #alleyto_t          ;tabela timeout referente ao caminho
				   lda       spawntemp
				   lsla      
				   ldb       [a,x]
				   lbne      no_new_enemy
